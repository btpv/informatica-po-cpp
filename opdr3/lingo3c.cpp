...
int main() {
  ...
  while (results != "+++++") {
    ...
    results = checkLetters(secretWord, Guess);

    vector<char> letters_in_secret_word;
    vector<char> letters_not_in_secret_word;

    // put the letters in the secret word into a vector
    // put the letters that are not in the secret word into the other vector
    for (int i = 0; i < results.length(); i++) {
      if (results[i] == '+' || results[i] == '/') {
        letters_in_secret_word.push_back(Guess[i]);
      } else {
        letters_not_in_secret_word.push_back(Guess[i]);
      }
    }
    ...
    for (int i = 0; i < results.length(); i++) {
      if (results[i] == '/') {
        correct_letter_wrong_position.push_back(Guess[i]);
        correct_letter_wrong_position_index.push_back(i);
      }
    }

    vector<string> remaining_words;
    // check for the remaining possible words
    for (string word : words) {
